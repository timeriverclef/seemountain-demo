...
function renderHome() {
  const viewpoint = getActiveViewpoint();
  els.activeViewpointName.textContent = state.location?.label ?? "等待定位";
  els.routeTitle.textContent = state.location
    ? `${state.location.label}`
    : "真实实验";
  els.routeDescription.textContent = state.location
    ? `当前使用 ${state.dataSource} 数据。点击真实实验可重新定位和刷新附近山峰。`
    : "点击真实实验后请求定位，并尝试从开放地图数据中获取附近山峰。";
  els.peakCount.textContent = String(state.peaks.length);
  els.homeHeading.textContent = String(Math.round(state.heading)).padStart(3, "0");
  els.dataSourceText.textContent = state.dataSource;
  els.scannerViewpoint.textContent = state.location?.label?.replace("西湖 · ", "") ?? viewpoint.name;
  els.locationText.textContent = state.location?.label ?? "等待定位";
}
  ...
  if (state.hasScanResults) {
    targets.forEach((target, index) => {
      ...
      button.innerHTML = `
        ...
        <span>${formatDistance(target.distance)} · ${target.peak.elevation}m · ${Math.round(target.confidence * 100)}%</span>
      `;
...
function computeTargets() {
  ...
  if (visible.length) return spreadLabels(visible);
...
function spreadLabels(targets) {
  const sorted = [...targets].sort((a, b) => a.x - b.x);
  for (let i = 1; i < sorted.length; i += 1) {
    const previous = sorted[i - 1];
    const current = sorted[i];
    if (Math.abs(current.x - previous.x) < 16 && Math.abs(current.y - previous.y) < 12) {
      current.y = clamp(previous.y + 12, 28, 68);
    }
  }
...
function renderMapFallback() {
  ...
  const peakPoints = state.peaks.length ? state.peaks : state.demoPeaks;
  const locationPoints = state.location ? [state.location] : state.viewpoints;
  ...
  const position = projectToMapFallback(state.location ?? getActiveViewpoint(), bounds);
...
function addMapMarkers() {
  ...
  const markerLocation = state.location ?? getActiveViewpoint();
  const marker = new window.maplibregl.Marker({ element: el })
...
function updateMapFocus() {
  if (!state.map) return;
  const center = state.location ?? getActiveViewpoint();
  state.map.jumpTo({
