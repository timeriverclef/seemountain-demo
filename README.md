...
> 当前版本不是完整图像识别，也不是真 ARCore/ARKit。它是纯静态 Web Demo，识别主链路仍是位置、朝向和山峰坐标；扫描时会实验性提取山脊轮廓来校准标签细线锚点。

## 功能

- 真实实验：请求浏览器定位，并尝试从 OSM/Overpass 获取附近山峰。
- 摄像头扫描：调用手机后置摄像头作为实时取景画面。
- 实时 AR 标签：点击“扫描”后播放扫描动画，随后保持相机画面实时流动，并根据手机朝向持续切换山峰标签。
- 标签锚点：优先显示 70° 视野内的中心主目标和最近山峰，并尝试把细线吸附到扫描帧中的山脊轮廓；失败时回退演示数据里的 `screenY`。
- 上传兜底：上传图片后进入静态画面识别流程，可用手动朝向校准查看标签变化。
